...
@AbilityManifest(name = "수호신", rank = Rank.L, species = Species.GOD, explain = {
		...
		" 되돌아오고 수호 스택에 비례해 적중 대상을 §e기절§f시키고 대미지가 증가합니다.",
		" 빛의 화살은 최대 §b$[MAX_ARROW]§f발까지 한 번에 발사 가능합니다.",
		"§7패시브 §8- §b사명§f: 살아있는 신 능력자 한 명당 수호 스택을 얻습니다.",
		" 수호 스택으로 얻은 효과는 신 종족에게 영향을 주지 못하며,",
		" 날 피해입힌 대상이 신 종족일 경우 대미지가 20% 경감됩니다.",
		"§7철괴 우클릭 §8- §e통찰§f: $[DURATION]초간 튕겨진 화살이 화살로부터 가장 가까운 대상에게로",
...
public class PatronSaint extends Synergy implements ActiveHandler {
	...
	protected void onUpdate(AbilityBase.Update update) {
	    if (update == AbilityBase.Update.RESTRICTION_CLEAR) {
	    	...
	    	actionbarChannel.update(Strings.repeat("§7▐", bullets).concat(Strings.repeat("§e▐", maxbullet - bullets)));
	...
	private AbilityTimer reload = new AbilityTimer(TaskType.NORMAL, reloadCount) {
		...
		protected void run(int count) {
			...
			reloadac.update("재장전: " + Strings.repeat("§a|", progress).concat(Strings.repeat("§7|", 15 - progress)));
		}
