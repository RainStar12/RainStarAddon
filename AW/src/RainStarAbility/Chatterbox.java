...
public class Chatterbox extends AbilityBase {
	
	public Chatterbox(Participant participant) {
		super(participant);
	}
	
	public static final SettingObject<Double> PERIOD = 
			abilitySettings.new SettingObject<Double>(Chatterbox.class, "damage-increase", 30.0,
			"# 문장이 나오는 주기", "# 단위: 초") {
		@Override
	...
	public static final SettingObject<Integer> DECREASE = 
			abilitySettings.new SettingObject<Integer>(Chatterbox.class, "damage-decrease", 75,
			"# 받아쓰기 중 피해량 감소", "# 단위: %") {
		@Override
	...
	static final String koreans = "계집 바뀐 건 모르고 젓가락 짝 바뀐 건 안다|"
			...
			+ "열 번 찍어 안 넘어갈 나무 없다|"
			+ "열 벙어리가 말을 해도 가만 있거라|"
			+ "열사람 형리를 사귀지 말고 한가지 죄를 범하지 말라|"
			+ "열 손가락 물어 안 아픈 손가락 없다|"
			+ "귀신은 경문에 막히고 사람은 인정에 막힌다|"
			+ "귀한자식 매한대 더 때리고 미운자식 떡한개 더 준다|"
			...
			+ "아들못난건 제집만 망하고 딸못난건 양사돈이 망한다|"
			+ "아랫돌 빼어 웃돌 괴기|"
			+ "아이 싸움이 어른 싸움된다|"
			+ "아침 놀 저녁 비요 저녁 놀 아침 비라|"
			+ "아홉살 일곱살 때에는 아홉동네에서 미움을 받는다|"
			+ "가는 세월 오는 백발|"
			+ "가난한 집 신주 굶듯 한다|"
			+ "가는 손님은 뒤꼭지가 예쁘다|"
			+ "가루는 칠수록 고와지고 말은 할수록 거칠어진다|"
			...
			+ "레인스타|"
			+ "단 13초 앞조차 모른다 해도|"
			+ "기분 내키는 대로 하는 난 적을 골라 싸우는 비겁한 소년|"
	...
	public AbilityTimer skill = new AbilityTimer(duration) {
		...
		@Override
		public void onStart() {
			getPlayer().sendMessage("§2[§a!§2] §b" + nowProverb);
		}
		
		@Override
		public void run(int count) {
			NMS.sendTitle(getPlayer(), count < 100 ? "§c§l" : "§a§l" + df.format(duration / 20.0), "", 0, 10, 0);
