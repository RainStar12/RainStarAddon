...
@AbilityManifest(
		...
		" 결정 속에 갇힌 플레이어들의 §d회복 효과§f는 대신 자신이 §e흡수 체력§f으로 변환해",
...
public class Fairy extends Synergy implements ActiveHandler {
	...
	private final Cooldown cooldown = new Cooldown(COOLDOWN.getValue(), 100);
				...
				new AbilityTimer(TaskType.NORMAL, 20) {
					...
					protected void onSilentEnd() {
						...
						if (maxHealth <= yellowHeart) {
							...
							NMS.setAbsorptionHearts(getPlayer(), (float) (yellowHeart + e.getAmount()));
						}
